...
makeLib::makeLib(string input, string output, string McInfoOut, string s_pdg, string s_phi, string s_region){
...
if     (s_region == "barrel") m_region = 0;
else if(s_region == "endcap") m_region = 1;
else{std::cout<<"Error, wrong region:"<<s_region<<std::endl;}
...
tree_out->Branch("m_HCAL_E", &m_HCAL_E   ); 
...
h2_x_Mom_all = new TH2D("h2_x_Mom_all"     ,"",160,1850,2010, 1000,0,1000);          
h2_x_Mom_have = new TH2D("h2_x_Mom_have"   ,"",160,1850,2010, 1000,0,1000);          
h2_x_Mom_ratio = new TH2D("h2_x_Mom_ratio" ,"",160,1850,2010, 1000,0,1000);          
...
bool makeLib::mutate(){
    ...
    for(long long i=0;i<tree_in->GetEntries();i++){
        ...
        if(m_region==0)//Barrel
        {
            ...
            for(int j=0;j<m_HcalHit_x->size(); j++)
            {
                m_HCAL_E += m_HcalHit_E->at(j);
            }
            ...
            ////////////
            /*
            float mom_tmp = 1000*sqrt( m_mc_Px->at(0)*m_mc_Px->at(0) + m_mc_Py->at(0)*m_mc_Py->at(0) + m_mc_Pz->at(0)*m_mc_Pz->at(0) ); 
            h2_x_Mom_all->Fill(m_mc_vertexX->at(0), mom_tmp);
            if(m_Tag==2) h2_x_Mom_have->Fill(m_mc_vertexX->at(0), mom_tmp);
            */
            ///////////
        }
        ...
        {
            if (m_mc_Px->size() != 1) continue;
            //if (m_mc_Pdg->at(0) != m_pdg || m_mc_Pz->at(0) < 0) continue;
            if (m_mc_Pdg->at(0) != m_pdg || m_mc_Pz->at(0) < 0 || m_mc_Px->at(0) < 0 || m_mc_Py->at(0) < 0 ) continue;//using symmetry
            mycout << float(m_mc_vertexZ->at(0)) <<" " << float(m_mc_Px->at(0)*1000) << " " << float(m_mc_Py->at(0)*1000) <<" " << float(m_mc_Pz->at(0)*1000) <<std::endl;      
            m_Tag = 1;
            if(m_Hit_x->size() == 0) m_Tag = 0;
            else if(m_HcalHit_x->size() > 0) m_Tag = 2;
            for(int j=0;j<m_HcalHit_x->size(); j++)
        ...
        else{std::cout << "Wrong region "<< endl; }
    }
    mycout.close();
