...
// reduce phi bin(-90 to 90, 90 bins) and theta bin (0 to 90, 45 bins)
// increase E bin from 500 MeV to 1 GeV
...
bool makeLib::mutate(){
    ...
    for(int i=0;i<tree_in->GetEntries();i++){
        ...
        int cos_phi_1 = int(acos(cos_phi)*180/PI)/2;
        int bin_phi = (m_mc_Py->at(0) > 0) ? cos_phi_1 : 45 + cos_phi_1;
        if(bin_phi>90 || bin_phi<0) continue;
        ...
        int bin_theta = int(acos(cos_theta)*180/PI)/2;
        if(bin_theta>45 || bin_theta<0) continue;
        int bin_E = int(mom*1000)-100;
        h_mc_mom  ->Fill( mom*1000 );
        if(bin_E>900 || bin_E<0) continue;
        int index = bin_phi*45*900 + bin_theta*900 + bin_E;
