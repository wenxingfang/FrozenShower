...
bool SLCIORdr::mutate(){
    ...
    if(lcEvent) {  //cout<<"det name="<< lcEvent->getDetectorName() <<endl;
                   ...
                   for(unsigned int i=0; i< colVec->size(); i++){
                       if(colVec->at(i)=="CaloToSimuCaloLink"){
                           ColVec  = lcEvent->getCollection(colVec->at(i));
                           cout<<"col name="<< colVec->at(i)<<",n ele="<<ColVec->getNumberOfElements()<<endl;
                           for (unsigned k = 0, N = ColVec->getNumberOfElements(); k < N; ++k)
                           {
                               auto rval = (EVENT::LCRelation *)ColVec->getElementAt(k);
                               //auto rFrom = (EVENT::CalorimeterHit *)rval->getFrom();
                               //auto rTo = (EVENT::SimCalorimeterHit *)rval->getTo();
                               //if(rFrom==0 || rTo==0) std::cout<<"rFrom="<<rFrom<<",rTo="<<rTo<<std::endl;
                               if(rval->getFrom()==0 || rval->getTo()==0) std::cout<<"rFrom="<<rval->getFrom()<<",rTo="<<rval->getTo()<<std::endl;
                           }
                       }
                  }
                 }
