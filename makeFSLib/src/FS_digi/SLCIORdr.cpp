...
tree_out->Branch("m_Hit_region", &m_Hit_region);
    ...
    if(lcEvent) {  //cout<<"det name="<< lcEvent->getDetectorName() <<endl;
                   ...
                   */
                   for(unsigned int i=0; i< colVec->size(); i++){
                       ...
                       //if(colVec->at(i)=="ECALBarrel" || colVec->at(i)=="HCALBarrel"){// consider only barrel now
                       if(colVec->at(i)=="ECALEndcap" || colVec->at(i)=="HCALEndcap"){// consider only barrel now
                       //if(colVec->at(i)=="ECALEndcap" || colVec->at(i)=="ECALOther"){// consider only barrel now
                       //if(colVec->at(i)=="EcalEndcapSiliconCollection"){// consider only endcap now
                       //if(colVec->at(i)=="HCALBarrel"){// consider only barrel now
                       //if(colVec->at(i)=="HCALEndcap"){// consider only barrel now
                           int region_flag = -1;
                           if(colVec->at(i)=="ECALEndcap") region_flag = 1;
                           if(colVec->at(i)=="HCALEndcap" ) region_flag = 2;
                           //if(colVec->at(i)=="ECALBarrel") region_flag = 1;
                           //if(colVec->at(i)=="HCALBarrel") region_flag = 2;
                           ColVec  = lcEvent->getCollection(colVec->at(i));
                           CellIDDecoder<CalorimeterHit> IdDecoder(ColVec);
                           ...
                           for(unsigned int kk=0; kk < ColVec->getNumberOfElements(); kk++){
                               m_Hit_region.push_back(region_flag);
                           }
                           //std::cout<<"Hi m_HitEn_tot="<<m_HitEn_tot<<std::endl;
                       }
                       /*
                       else if(colVec->at(i)=="HCALBarrel"){// consider only barrel now
...
bool SLCIORdr::getSimHitInfo(CellIDDecoder<CalorimeterHit> & Hit_idDecoder, LCCollection* Col, std::vector<int>& vec_ID0, std::vector<int>& vec_ID1, std::vector<double>& vec_Hit_x, std::vector<double>& vec_Hit_y, std::vector<double>& vec_Hit_z, std::vector<double>& vec_Hit_E , double& Hit_cm_x, double& Hit_cm_y, double& Hit_cm_z, double& Hit_tot_e, std::vector<int>& vec_ID_S, std::vector<int>& vec_ID_M, std::vector<int>& vec_ID_I, std::vector<int>& vec_ID_J, std::vector<int>& vec_ID_K){
      if(Col)
      { 
         //std::cout<<"Hi getHitInfo"<<std::endl;
         double tot_E = 0;
         double sum_x = 0;
         double sum_y = 0;
         double sum_z = 0;
         int NHit = Col->getNumberOfElements();
         for( int i=0; i<NHit; i++ ){
           //EVENT::CalorimeterHit* in = dynamic_cast<EVENT::CalorimeterHit*>(Col->getElementAt(i));
           IMPL::CalorimeterHitImpl* in = dynamic_cast<IMPL::CalorimeterHitImpl*>(Col->getElementAt(i));
          //cout<<"Hit "<<i <<",CellID0="<<in->getCellID0()<<", CellID1="<<in->getCellID1()<<", E= "<<in->getEnergy()<<" GeV, x="<<in->getPosition()[0]<<",y="<<in->getPosition()[1]<<",z="<<in->getPosition()[2]<<"\n";
          int StaveNum = Hit_idDecoder(in)["S-1"];// from 0 - 7
          int MNum     = Hit_idDecoder(in)["M"  ];// from 1 - 5
