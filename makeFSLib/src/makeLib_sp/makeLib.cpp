...
bool makeLib::mutate(){
    ...
    const int E_bin=900;
    const int bins = phi_bin*theta_bin*E_bin;
    const float v_Y_cut=500;
    ...
    for(int i=0;i<tree_in->GetEntries();i++){
        ...
        if (m_mc_Px->at(0) < 0 || m_mc_Pz->at(0) < 0 || abs(m_mc_vertexY->at(0)) > v_Y_cut) continue;
        ...
        if(cos_phi_1>=25) continue;
        h_mc_phi  ->Fill( (m_mc_Py->at(0) > 0) ? cos_phi_1 : -cos_phi_1 );
        int bin_phi = (m_mc_Py->at(0) > 0) ? cos_phi_1 : 25 + cos_phi_1;
        if(bin_phi>=phi_bin || bin_phi<0) continue;
        ...
        h_mc_theta  ->Fill( acos(cos_theta)*180/PI );
        int bin_theta = abs(int(acos(cos_theta)*180/PI) - 90 );
        if(bin_theta>=theta_bin || bin_theta<0) continue;
        int bin_E = int(mom*1000)-100;
        h_mc_mom  ->Fill( mom*1000 );
        if(bin_E>=E_bin || bin_E<0) continue;
        int index = bin_phi*theta_bin*E_bin + bin_theta*E_bin + bin_E;
        if(index<0 || index>=bins) continue;
        int tag = 1;
        if(m_Hit_x->size() == 0) tag = 0;
        else if(m_HcalHit_x->size() > 0) tag = 2;
        
        h_mc_v_x  ->Fill(  m_mc_vertexX->at(0) );
        h_mc_v_y  ->Fill(  m_mc_vertexY->at(0) );
        h_mc_v_z  ->Fill(  m_mc_vertexZ->at(0) );
        //if(tmp_Tag.at(index).size() != 0) continue;// only one event each bin
        vector<float> tmp_x;
        vector<float> tmp_y;
        vector<float> tmp_z;
        vector<float> tmp_E;
        ...
        for(int j=0;j<m_Hit_x->size(); j++)
        {
            ...
            tmp_J.push_back(new_J);//need more check
